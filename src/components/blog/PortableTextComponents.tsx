...
export const portableTextComponents: PortableTextComponents = {
  block: {
    h2: ({ children }) => (
      <h2 className="font-[var(--font-heading)] text-2xl md:text-3xl text-text-primary mt-10 mb-4">{children}</h2>
    ),
    h3: ({ children }) => (
    ...
    normal: ({ children }) => <p className="text-text-secondary leading-relaxed mb-5">{children}</p>,
    blockquote: ({ children }) => (
      <blockquote className="border-l-2 border-accent pl-5 my-6 text-text-tertiary italic">{children}</blockquote>
    ),
  },
  marks: {
  ...
  types: {
    image: ({ value }) => (
      <figure className="my-8">
        ...
        />
        {value.caption && (
          <figcaption className="mt-2 text-sm text-text-tertiary text-center">{value.caption}</figcaption>
        )}
      </figure>
    ),
    code: ({ value }) => (
    ...
    break: () => <hr className="my-10 border-none h-px bg-border" />,
  },
  list: {
    bullet: ({ children }) => <ul className="list-disc pl-6 mb-5 space-y-1.5 text-text-secondary">{children}</ul>,
